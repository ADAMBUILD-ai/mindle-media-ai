...
def stage_snapshot(model: str, source: Path, expected_sha256: str) -> dict:
    ...
    return {"model": model, "revision": SNAPSHOTS[model]["revision"], "source": str(source), "target": str(target), "weight_sha256": actual_sha256}


def stage_realesrgan(source: Path, expected_sha256: str) -> dict:
    ...
    return {"model": "realesrgan", "revision": SNAPSHOTS["realesrgan"]["revision"], "source": str(source), "target": str(target), "release_sha256": actual_sha256, "model_sha256": sha256(target / "real_esrgan_x4plus.onnx")}
...
def main() -> None:
    ...
    record = stage_realesrgan(args.source, args.expected_sha256) if args.model == "realesrgan" else stage_snapshot(args.model, args.source, args.expected_sha256)
